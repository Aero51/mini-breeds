...
why that approach was chosen over the alternatives. Read it top to bottom — it
goes in dependency order, so nothing refers to something you haven't met yet.

This is the **no-images** variant of the app: there are no network images
anywhere by design — every breed is represented by a deterministic monogram
avatar instead. Everything else matches the main branch.
...
(Honest note: `map` currently has no production caller on this branch — the
DTO mapper builds its `Success` directly — but it completes the result API,
is unit-tested, and costs four lines.)
...
### `BreedsResponseDto` (data class)

**What it does:** Mirrors the JSON of `GET api/breeds/list/all`:
`{ "message": { "bulldog": ["boston", …], … }, "status": "success" }`. The
`message` keys are breed names; the values are (possibly empty) sub-breed
lists.

**Why this approach:** One endpoint, one concrete DTO. Default values
(`emptyMap()`, `""`) mean a missing field parses instead of crashing — the
empty `status` then fails the status check downstream rather than silently
passing.

**Alternatives rejected:**
- *A generic `DogResponseDto<T>` envelope* — the right call when several
  endpoints share the message-plus-status shape (the main branch, with its
  image endpoint, uses exactly that). With a single endpoint there is no
  duplication to remove, so generics would be machinery without a payoff.
- *Parsing into the domain `Breed` directly* — couples the domain model to
  the wire format (see `Breed` above).
- *Parsing with `JsonElement` and picking fields dynamically* — trades
  compile-time field checking for runtime casting.

### `DogApiService` (interface)

**What it does:** Declares the single HTTP endpoint as a Kotlin function.
Retrofit reads the `@GET` annotation and generates the actual HTTP-calling
...
- `refreshBreeds()` — runs on the IO dispatcher; chains
  `safeApiCall { api.getAllBreeds() }` → `toBreeds()` → `onSuccess` (store in
...
- `favorites` / `toggleFavorite(name)` — straight delegation to the favorites
  ...
  knock on, not two.
- `toBreeds()` (private) — checks dog.ceo's own `"status"` field, then maps
  the raw name→sub-breeds map into sorted `Breed` objects. The API can return
  HTTP 200 with `"status": "error"` in the body; without this check that
  would look like a successful-but-empty response.
...
**What it does:** A colored circle with the breed's first letter — the app's
entire visual identity system, since this variant loads no images at all.
The background hue is derived from the breed name's hash code, so "akita" is
always the same color everywhere, and the saturation/lightness adapt to dark
mode. The computed colors are `remember`ed so they aren't recalculated on
every recomposition.

**Why this approach:** The list API gives no images, and fetching a photo per
breed would mean ~100 extra network calls on launch against a free API. A
deterministic monogram gives each breed identity for free, offline,
instantly — and keeps the app fully functional with zero image
infrastructure (no image-loading library, no extra failure modes).
...
- `uiState` — `combine` of three flows: the observed breed from the cache,
  favorites, and `refreshError`. Priority: breed available → `Content`; no
  breed but an error → `Error`; otherwise `Loading`.
...
/ `BreedDetailContent`. `BreedHeader` shows the large monogram avatar, the
capitalized name, and the sub-breed count. `SubBreedCard` renders each
sub-breed (cards, not navigable — sub-breeds have no further data in this
API). `BreedDetailTestTags` again holds test IDs.
...
- **ALTERNATIVES.md** — per class: the alternative implementations
  considered, their trade-offs, and the deciding factor.
- **NETWORKLAYER.md / ERRORHANDLING.md** — deep dives on those slices.
