...
This is the **no-images** variant: the app deliberately loads no network
images, anywhere. That decision itself is covered under `BreedAvatar` below.
...
- *Arrow's `Either<AppError, T>`* — richer combinators, well-tested. Lost
  because pulling in a functional-programming library for one type is
  dependency weight with no payoff; the three extensions cover every
  operation the app actually performs.
...
— speculative API is a cost, not an investment. `map` keeps a place despite
having no production caller on this branch: it completes the API and is
pinned by tests.)
...
`observeBreed(name)`, `favorites`, `toggleFavorite(name)`.
...
**Chosen:** one concrete `@Serializable` DTO for the single endpoint
(`message: Map<String, List<String>> = emptyMap()`, `status: String = ""`).

**Alternatives:**
- *A generic `DogResponseDto<T>` envelope* — the right call when several
  endpoints share dog.ceo's message-plus-status shape; the main branch (which
  also calls the random-image endpoint) uses exactly that. With one endpoint
  there is no duplication for generics to remove — a type parameter would be
  machinery without a payoff. If a second endpoint is ever added, collapse to
  the generic envelope then.
- *Parsing with `JsonElement` and picking fields dynamically* — no DTO class
  at all. Lost because it trades compile-time field checking for runtime
  casting, the exact opposite of what kotlinx.serialization buys.
- *No field defaults* — a missing field would fail parsing loudly. Lost
  narrowly: with defaults, a missing `status` becomes `""` and still fails
  the explicit status check downstream, so nothing is silently accepted,
  and `coerceInputValues` can handle nulls without a crash.

**Deciding factor:** one endpoint, one concrete class — the simplest shape
that parses the spec payload.

### `DogApiService`

**Chosen:** Retrofit interface, one `suspend` endpoint.
...
- *A Retrofit `CallAdapter` returning `AppResult<T>` automatically* — most
  ...
  function call per endpoint. At one endpoint, the arithmetic says wrapper.
...
`safeApiCall → toBreeds() → onSuccess(cache) → onFailure(log)`; `toBreeds()`
doing the `status`-field check and the DTO→domain mapping in one place.
...
### `BreedAvatar` — and the no-images decision itself

**Chosen:** monogram circle, hue from `name.hashCode()`, dark-mode-aware,
`remember`ed — the app's entire visual identity, with no image loading
anywhere.

**Alternatives:**
- *A photo per breed (list rows)* — prettier. Lost: the list endpoint
  provides no image URLs, so this would mean ~100 extra calls against a free
  API on first launch, plus jank and placeholder churn.
- *A photo on the detail screen only* — the bonus the main branch implements
  (one extra endpoint, Coil, and graceful degradation back to this very
  monogram). This variant deliberately skips it: zero image infrastructure
  means one fewer library, no image failure modes, and a fully offline-capable
  UI once the breed list is cached. Which variant is "right" depends on
  whether visual polish or minimal surface is being optimized for.
...
**Deciding factor:** per-breed identity at zero network cost.
...
+ refreshError; cold-cache refresh in `init`.

**Alternatives:**
- *Refetch unconditionally in `init`* — simpler condition. Lost: a pointless
  network call on every navigation with a warm cache.
- *No detail ViewModel at all (read the argument in the composable)* — the
  minimal take-home shape. Lost: the screen needs the cache observation,
  favorite reactivity, and the cold-cache refresh; that's ViewModel work,
  not composable work.
- *Shared ViewModel with the list* — see navigation section.

**Deciding factor:** the detail screen must survive process death and react
to favorite changes; both need a ViewModel observing the repository.
...
- **MockWebServer over mocking the API interface** — mocking `DogApiService`
  ...
  stack.
- **No image-loading library at all** — this variant's defining choice; see
  `BreedAvatar` above for the full trade-off against the photo-bearing main
  branch.
